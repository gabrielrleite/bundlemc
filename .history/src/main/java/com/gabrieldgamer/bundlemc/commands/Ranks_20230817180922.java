...
public class Ranks implements CommandExecutor {
    ...
    private String checkValue(Double d, Double Value) {
        if (Value >= d) {
            return "§a✔";
        }
        return "§4✖";
    ...
    private String checkVisitante(Double a, Double A, Double b, Double B, Double c, Double C) {
        if (a >= A && b >= B && c >= C) {
    ...
    private String checkRank(String suffix) {
    ...
    public boolean onCommand(CommandSender sender, org.bukkit.command.Command command, String label, String[] args) {
        ...
        // Double playtime_ticks =
        // Double.valueOf(player.getStatistic(Statistic.PLAY_ONE_MINUTE)) / 20.0 / 60 /
        // 60;
        // Double walkedCm = player.getStatistic(Statistic.WALK_ONE_CM) / 100.0 /
        // 1000.0;
        // DecimalFormat d = new DecimalFormat("#.##");
        Double playerMoney = 100.00;// instance.economy.getBalance(player);
        Double playerTime = 1.2;// Double.valueOf(d.format((playtime_ticks)));
        Double playerWalk = 1.3;// Double.valueOf(d.format(walkedCm));
        ...
        ItemStack veterano_item = new ItemBuilder(Material.DIAMOND_CHESTPLATE)
                .setName("Veterano");
        ItemStack voltar_item = new ItemBuilder(Material.FILLED_MAP)
                .setName("Voltar");
